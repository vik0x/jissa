		...
		<script>
			$(function(){
				$('.activo').on('change', function(){
					...
					$.ajax({
						url:'/administrador/estatus/idioma.html',
						type:'POST',
						data:{
							_token:"{{csrf_token()}}",
							id:id
			...
			<!-- START PANEL -->
				<div class="panel panel-transparent">
					<div class="panel-heading">
						<div class="panel-title">Idiomas para Usuario
						</div>
						<div class="pull-right">
							<div class="col-xs-12">
		                    	<a href="{{url('/administrador/agregar/idioma.html')}}" class="btn btn-primary btn-cons"><i class="fa fa-plus"></i> Agregar</a>
			              	</div>
		                </div>
					</div>
							...
							<div id="detailedTable_wrapper" class="dataTables_wrapper form-inline no-footer">
								<table class="table table-hover table-condensed table-detailed dataTable no-footer" id="detailedTable" role="grid">
									...
									<tbody>
										...
										<tr role="row" class="">
											<td class="v-align-middle">{{$idioma->nombre}}</td>
											<td class="v-align-middle"><input type="checkbox" {{$idioma->estatus == 1 ? "checked" : ""}} class="activo" data="{{$idioma->id_idioma}}" data-toggle="toggle"></td>
											<td class="v-align-middle">
												<div class="btn-group btn-group-justified">
						                            <div class="btn-group">
						                              	<form action="{{url('/administrador/modificar/idioma' . $idioma->id_idioma . '.html')}}" method="post">
															{!!csrf_field()!!}
						                            ...
						                            <div class="btn-group">
						                            	<form action="{{url('/administrador/eliminar/idioma' . $idioma->id_idioma . '.html')}}" method="post" class="del_element">
															{!!csrf_field()!!}
